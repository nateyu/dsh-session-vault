...
/**
 * Sessions the workspace sidebar hides: archived, or blank (no turn yet),
 * excluding subagents. The current blank "New Session" row stays in the
 * sidebar but is still listed here so it can be managed after switching away.
 * @param {{ origin?: string, archived: boolean, blank: boolean }} session
 * @returns {boolean}
 */
export function isHiddenFromSidebar(session) {
  if (session.origin === 'subagent') return false
  return session.archived === true || session.blank === true
}
...
export async function sessionIsBlank(ctx, record, signal) {
  ...
  if (live !== undefined) return eventsAreBlank(live.events ?? [])
...
/**
 * List sessions the sidebar hides (archived or blank), excluding subagents.
 * @param {object} ctx
 * @param {AbortSignal | undefined} signal
 */
export async function listVaultSessions(ctx, signal) {
  ...
  const rows = await mapPool(candidates, INSPECT_CONCURRENCY, async (record) => {
    ...
    const blank = await sessionIsBlank(ctx, record, signal)
    return {
  ...
  const hidden = rows.filter((row) => isHiddenFromSidebar(row))
  const observations = hidden.length === 0
    ? []
    : await ctx.sessionQuery.readTitleSnapshots(hidden.map((row) => row.sessionId), signal)
  const titles = titleMapFromObservations(observations)
